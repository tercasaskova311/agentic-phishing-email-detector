...
def setup_kaggle_gpu():
    ...
    if torch.cuda.is_available():
        ...
        raise RuntimeError("GPU not available! => GPU in Kaggle settings.")
    ...
    try:
        ...
        print(f"HuggingFace token not found: {e}")
    
    torch.backends.cudnn.benchmark = True
    print()

# DATASET CLASS
class EmailDataset(Dataset): #PyTorch Dataset for efficient batching
...
# MODEL LOADER ==================================================
class ModelLoader: #Handles model loading with Kaggle GPU optimizations
    ...
    def load(model_name: str, use_4bit: bool = False):
        ...
        tokenizer = AutoTokenizer.from_pretrained(
            ...
            padding= left # for all models we are applying this is left
        )
        ...
        generator = pipeline(
            ...
            tokenizer=tokenizer,
            device_map='auto',
            batch_size=CONFIG['batch_size']  # Set default batch size - 16
        )
...
class PhishingDetector:
    ...
    def analyze_batch(self, prompts: List[str]) -> List[Tuple[str, str]]: #batching for efficient GPU usage IN KAGGLE NOTEBOOK
...
def main():
    ...
    print("PHISHING DETECTION - ZERO-SHOT")
    print("="*70)
