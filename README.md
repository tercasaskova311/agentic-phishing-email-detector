...
Detect phishing emails using LLMs with single-agent, multi-agent, and debating architectures, alongside traditional ML baselines (Logistic Regression, Naive Bayes, Random Forest) for comparison.

This repository compares traditional ML, single LLMs, multi-agent debate systems, ensemble methods, and fine-tuning for phishing email detection. The goal is to see how close LLM approaches can get to the 98-99% accuracy achieved by classical ML.
...
- LLM-based detection with semantic context awareness
- Multi-agent and debating systems with consensus-style decisions
- Prompt engineering for zero-shot and few-shot modes
- Traditional ML baselines for fast, high-accuracy comparison
...
- Kaggle notebooks for GPU-backed experiments

## Architecture

- **Single-Agent**: Zero-shot / few-shot prompts
- **Multi-Agent**: Email Agent -> URL Agent -> Pattern Agent -> Judge Agent
- **Debating Agents**: Multiple LLMs vote in parallel, weighted consensus

## Datasets

- ENRON (primary results in this repository)
- AIGEN (referenced for LLM-generated phishing content)
- TREC Spam Corpus (referenced for classic spam benchmarks)
...
├── docs/                          # Methodology documentation (consolidated)
├── first_datasets/                # Raw datasets
├── models/                        # Agents and ML baselines
...
cd agentic-phishing-email-detector
```
...
# 4. Hugging Face token - get access to models like Llama and Qwen
