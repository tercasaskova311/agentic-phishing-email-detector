#!/usr/bin/env python3
"""
Phase 2: Preprocessing Phishing Dataset
Format: sender, receiver, date, subject, body, urls, label
Goal: Extract 1.5k phishing emails for balanced dataset
"""
...
def preprocess_phishing():
    ...
    print("="*60)
    print("PHASE 2: PREPROCESSING PHISHING DATASET")
    print("="*60)
    
    # Load dataset
    print("\n1. Loading dataset...")
    df = pd.read_csv(DATA_DIR / "phishing.csv")
    ...
    print(f"   Note: This dataset should contain only phishing emails (label=1)")
    
    # Verify all are phishing
    if (df['label'] == 1).all():
        print(f"   ✓ Confirmed: All {len(df)} emails are phishing")
    else:
        print(f"   ⚠ Warning: Found {(df['label'] != 1).sum()} non-phishing emails")
        df = df[df['label'] == 1]
        print(f"   Filtered to {len(df)} phishing emails")
    ...
    # Sample 1.5k phishing emails
    print("\n4. Sampling 1.5k phishing emails...")
    n_samples = 1500
    
    if len(df) >= n_samples:
        sample_df = df.sample(n=n_samples, random_state=42)
        print(f"   ✓ Sampled {len(sample_df)} phishing emails")
    else:
    ...
    # Save
    output_file = RESULTS_DIR / "phishing_preprocessed_1.5k.csv"
    ...
    print(f"   All phishing: {(final_df['label'] == 1).all()}")
    ...
    print("\n" + "="*60)
    print("PHISHING PREPROCESSING COMPLETE!")
    print("="*60)
